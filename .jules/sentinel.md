...
## 2025-06-28 - [Overly Permissive CORS Configuration]
**Vulnerability:** Found `app.enableCors()` with empty arguments in `apps/api/src/main.ts`. By default, this allows any origin (`*`) to access the API endpoints in the browser, which can lead to unauthorized cross-origin requests.
**Learning:** Overly permissive CORS settings are a common oversight during development that can easily leak into production. They open the API to CSRF-like attacks and unauthorized data reading if not restricted properly.
**Prevention:** Always configure explicit CORS settings within `app.enableCors()`. Specify the allowed origins (e.g., using `process.env.FRONTEND_URL` with a localhost fallback for development), methods, and credentials, rather than relying on the permissive default.
