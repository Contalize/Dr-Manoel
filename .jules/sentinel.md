...
## 2025-05-24 - [Hardcoded JWT Secret Fallback]
**Vulnerability:** Found `process.env.JWT_SECRET || 'super-secret'` being used in `apps/api/src/auth/auth.module.ts` and `apps/api/src/auth/jwt.strategy.ts`. This allows the application to fall back to a widely-known, weak secret if the environment variable is missing, which could let attackers forge JWT tokens and gain unauthorized access to the API.
**Learning:** This is a common local development pattern that accidentally gets committed and poses a massive critical risk in production environments if environment variables are ever misconfigured or dropped.
**Prevention:** Never use hardcoded fallback secrets for cryptographic functions or JWT signing. Always fail-fast (e.g., throwing an error at startup) if required security environment variables are not provided.
